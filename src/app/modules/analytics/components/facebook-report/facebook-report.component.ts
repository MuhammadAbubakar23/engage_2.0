...
totalMessageAndrepliesCount:any
  ...
  ngOnInit(): void {
    const newObj = { title: 'Facebook Report', url: '/analytics/feacbook-report' };
...
if(!this.totalPeopleWhoViewSpanDates.includes(x.dateValue.split('T')[0])){
...
if(!this.pageReactionsSpanDates.includes(x.totalReactionsDateValue.split('T')[0])){
  this.pageReactionsSpanDates.push(x.totalReactionsDateValue.split('T')[0])
}
...
this.agentMessagesData . forEach((x:any)=>{
  debugger
  this.totalMessageAndrepliesCount=x.totalMessagesAndRepliesCount
  this.totalMessagePostCommentsAndreplies=x.totalPostCommentsAndReplies
})
// Publishin Behavior
...
this.publishedPegePostSpan ?.forEach((x:any)=>{
 ...
 if(!this.publishedPegePostSpanDates.includes(x.dateValue.split('T')[0])){
...
this.publishedUserPostSpan ?.forEach((x:any)=>{
  ...
  if(this.publishedUserPostSpanDates.includes(x.dateValue.split('T')[0])){
    this.publishedPegePostSpanDates.push(x.dateValue.split('T')[0])
  }
...
this.malepersentage=Math.floor((this.totalfemaleCount / this.totalmaleandfemalCount) * 100)
this.femalepersentage=Math.floor((this.totalmaleCount/this.totalmaleandfemalCount) *100)
// RecentPost
...
option = {
  ...
  legend: {
    data: ['Love', 'Anger', 'Care', 'Haha', 'Like','Pride','Sad','Sorry','ThankFul','Wow']
  },
  grid: {
    left: '3%',
    right: '4%',
