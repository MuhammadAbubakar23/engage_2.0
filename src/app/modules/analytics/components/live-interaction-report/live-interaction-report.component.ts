...
@Component({
  standalone:true,
  selector: 'app-live-interaction-report',
  templateUrl: './live-interaction-report.component.html',
  styleUrls: ['./live-interaction-report.component.scss'],
  imports:[SharedModule,FormsModule,NgxSpinnerModule,CommonModule]
})
export class LiveInteractionReportComponent implements OnInit {
  ...
  currentdate:any
  perviousInterationResponse:any
  clearsetTime:any
  constructor(
  ...
  ngOnInit(): void {
    console.log(this.formatedateTime);
  ...
  this.formatDate(this.currentDate)
    ...
    const newObj = {
      title: 'Live Interaction Report',
      url: '/analytics/live-interaction-report',
    };
  ...
  GetStatsInteractionReport() {
    ...
    // }
    const today=new Date()
    this.endDate=this.formatedateTime
    this.startDate=this.datePipe.transform(today,'yyyy-MM-dd')+ 'T00:00:00.000Z'
    const formData = {
    ...
    this.commonData.GetPreviousInteractionDashboard(formData).subscribe((response:any)=>{
      this.data = response.previousAgentPerformance;
      this.perviousInterationResponse=response
    //  this.socialMediaData = response.plateFormWiseInteraction;
...
setTimeout(() => {
  this.commonData.GetInteractionReport(formData).subscribe((res: any) => {
    ...
    this.previousSocialMediaData = this.perviousInterationResponse?.previousPlateFormWiseInteraction;
    ...
    this.LastfinalAverage = this.calculateAverageTime(
      this.perviousInterationResponse?.previousAgentPerformance,
...
error=>{
  ...
  formatDate(date:any) {
    ...
    this.formatedateTime= `${year}-${month}-${day}T${hours}:${minutes}:${seconds}.${milliseconds}Z`;
