...
export class BotMonitoringComponent implements OnInit {
  ...
  getChatDetails(activeChat: any) {
    {
      ...
      if (this.chats.length > 0) {
        ...
        const existingChatIndex = this.chats.findIndex(
          (chat) => chat[0].customer?.phone === activeChat.from
        );
        console.log('this.chats', this.chats);

        if (existingChatIndex != -1) {
          const isEqual =
            this.chats[existingChatIndex][0]['completed'] ===
  ...
  ngOnInit(): void {
    this.newChatIdSubscription = this._chatVisibilityS.newChatId$.subscribe(
      (newChat: any) => {
        if (newChat) {
                      ...
                      this.currentActiveChats.forEach((ch) => {
                        this._botMonitorS
                          ...
                          .subscribe((res) => {
                            const existingChatIndex = this.chats.findIndex(
                              (chat) =>
                                chat[0].customer?.phone ===
                                ch.customerIdentifier
                            );
                            const minimizeToggle =
                              this.chats[existingChatIndex][0]['isMinimized'];
                            const completed =
                              this.chats[existingChatIndex][0]['completed'];
